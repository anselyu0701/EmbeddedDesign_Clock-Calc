...
#line 64 "..\\Drivers\\STM32F1xx_HAL_Driver\\Src\\stm32f1xx_hal_crc.c"
...
HAL_StatusTypeDef HAL_CRC_DeInit(CRC_HandleTypeDef *hcrc)
{
  ...
  ((((hcrc)->Instance ->CR) |= ((0x1U << (0U)))));

   
  hcrc->State = HAL_CRC_STATE_RESET;

   
  do{ (hcrc)->Lock = HAL_UNLOCKED; }while (0);
...
uint32_t HAL_CRC_Accumulate(CRC_HandleTypeDef *hcrc, uint32_t pBuffer[], uint32_t BufferLength)
{
  ...
  do{ if((hcrc)->Lock == HAL_LOCKED) { return HAL_BUSY; } else { (hcrc)->Lock = HAL_LOCKED; } }while (0);
