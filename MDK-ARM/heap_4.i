		...
		if( ( xWantedSize & xBlockAllocatedBit ) == 0 )
		{
			...
			if( xWantedSize > 0 )
			{
				...
				if( ( xWantedSize & ( 0x0007 ) ) != 0x00 )
				{
					...
					if ((( xWantedSize & ( 0x0007 ) ) == 0) == 0) {ulPortSetInterruptMask(); for( ;; );};
				}
				else
				{
					;
			...
			if( ( xWantedSize > 0 ) && ( xWantedSize <= xFreeBytesRemaining ) )
			{
				...
				if( pxBlock != pxEnd )
				{
					...
					if( ( pxBlock->xBlockSize - xWantedSize ) > ( ( size_t ) ( xHeapStructSize << 1 ) ) )
					{
						...
						if ((( ( ( size_t ) pxNewBlockLink ) & ( 0x0007 ) ) == 0) == 0) {ulPortSetInterruptMask(); for( ;; );};
...
#line 295 "..\\Middlewares\\Third_Party\\FreeRTOS\\Source\\portable\\MemMang\\heap_4.c"

	if ((( ( ( uint32_t ) pvReturn ) & ( 0x0007 ) ) == 0) == 0) {ulPortSetInterruptMask(); for( ;; );};
	return pvReturn;
}
...
BlockLink_t *pxLink;
	...
	if( pv != 0 )
	{
		...
		if ((( pxLink->xBlockSize & xBlockAllocatedBit ) != 0) == 0) {ulPortSetInterruptMask(); for( ;; );};
		if ((pxLink->pxNextFreeBlock == 0) == 0) {ulPortSetInterruptMask(); for( ;; );};
